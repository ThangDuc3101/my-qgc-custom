...
void GeoFenceController::_managerVehicleChanged(Vehicle* managerVehicle) {
    ...
    (void) connect(_managerVehicle, &Vehicle::capabilityBitsChanged, this, [this](uint64_t) { emit supportedChanged(supported()); });
    (void) connect(_managerVehicle, &Vehicle::requestProtocolVersion, this, [this](unsigned) { emit supportedChanged(supported()); });
...
bool GeoFenceController::load(const QJsonObject& json, QString& errorString) {
    ...
    QList<JsonHelper::KeyValidateInfo> keyInfoList = {
                                                      { JsonHelper::jsonVersionKey, QJsonValue::Double, true }, { _jsonCirclesKey, QJsonValue::Array, true },
                                                      { _jsonPolygonsKey, QJsonValue::Array, true }, { _jsonBreachReturnKey, QJsonValue::Array, false },
                                                      };
    if (!JsonHelper::validateKeys(json, keyInfoList, errorString)) return false;
    ...
    for (const QJsonValue jsonPolygonValue: jsonPolygonArray) {
        if (jsonPolygonValue.type() != QJsonValue::Object) { errorString = tr("GeoFence polygon not stored as object"); return false; }
        QGCFencePolygon* fencePolygon = new QGCFencePolygon(false, this);
        if (!fencePolygon->loadFromJson(jsonPolygonValue.toObject(), true, errorString)) return false;
    ...
    for (const QJsonValue jsonCircleValue: jsonCircleArray) {
        if (jsonCircleValue.type() != QJsonValue::Object) { errorString = tr("GeoFence circle not stored as object"); return false; }
        QGCFenceCircle* fenceCircle = new QGCFenceCircle(this);
        if (!fenceCircle->loadFromJson(jsonCircleValue.toObject(), errorString)) return false;
        _circles.append(fenceCircle);
    }
    if (json.contains(_jsonBreachReturnKey)) {
        if (!JsonHelper::loadGeoCoordinate(json[_jsonBreachReturnKey], true, _breachReturnPoint, errorString)) return false;
...
void GeoFenceController::save(QJsonObject& json) {
    ...
    for (int i=0; i<_polygons.count(); i++) {
        ...
        _polygons.value<QGCFencePolygon*>(i)->saveToJson(jsonPolygon);
    ...
    for (int i=0; i<_circles.count(); i++) {
        ...
        _circles.value<QGCFenceCircle*>(i)->saveToJson(jsonCircle);
    ...
    if (_breachReturnPoint.isValid()) {
        ...
        JsonHelper::saveGeoCoordinate(_breachReturnPoint, true, jsonCoordinate);
...
void GeoFenceController::removeAllFromVehicle(void) {
    if (_masterController->offline() || syncInProgress()) return;
...
void GeoFenceController::updateSafePolygonPath(QVariantList safePath) {
    _safePolygonPath.clear();
    for(const QVariant& v : safePath) {
        if (v.canConvert<QGeoCoordinate>()) {
            _safePolygonPath.append(v.value<QGeoCoordinate>());
        }
    }
}

void GeoFenceController::sendToVehicle(void) {
    if (_masterController->offline() || syncInProgress()) {
        return;
    }
    qCDebug(GeoFenceControllerLog) << "GeoFenceController::sendToVehicle (Modified with safe path)";

    QmlObjectListModel safePolygonsToSend;
    if (!_safePolygonPath.isEmpty() && _polygons.count() > 0) {
        bool inclusion = _polygons.value<QGCFencePolygon*>(0)->inclusion();
        QGCFencePolygon* safePolygon = new QGCFencePolygon(inclusion, this);
        for(const QGeoCoordinate& coord : _safePolygonPath) {
            safePolygon->appendVertex(coord);
        }
        safePolygonsToSend.append(safePolygon);
    } else {
        // Dự phòng: Nếu không có hàng rào an toàn, gửi hàng rào gốc.
        // Điều này đảm bảo chương trình không gửi một danh sách rỗng nếu có lỗi xảy ra.
        qCWarning(GeoFenceControllerLog) << "Safe polygon path is empty, sending original polygons instead.";
        for(int i=0; i < _polygons.count(); i++) {
            safePolygonsToSend.append(new QGCFencePolygon(*_polygons.value<QGCFencePolygon*>(i), this));
        }
    }
    _geoFenceManager->sendToVehicle(_breachReturnPoint, safePolygonsToSend, _circles);
    setDirty(false);
}
...
void GeoFenceController::setDirty(bool dirty) {
    if (dirty != _dirty) {
        _dirty = dirty;
        if (!dirty) {
            for (int i=0; i<_polygons.count(); i++) _polygons.value<QGCFencePolygon*>(i)->setDirty(false);
            for (int i=0; i<_circles.count(); i++) _circles.value<QGCFenceCircle*>(i)->setDirty(false);
        }
        emit dirtyChanged(dirty);
    }
}
void GeoFenceController::_polygonDirtyChanged(bool dirty) { if (dirty) setDirty(true); }
...
void GeoFenceController::_setFenceFromManager(const QList<QGCFencePolygon>& polygons, const QList<QGCFenceCircle>& circles) {
    _polygons.clearAndDeleteContents();
    _circles.clearAndDeleteContents();
    for (const auto& poly : polygons) _polygons.append(new QGCFencePolygon(poly, this));
    for (const auto& circle : circles) _circles.append(new QGCFenceCircle(circle, this));
    setDirty(false);
}
...
void GeoFenceController::_managerSendComplete(bool error) { if (!error && _flyView) showPlanFromManagerVehicle(); }
void GeoFenceController::_managerRemoveAllComplete(bool error) { if (!error) showPlanFromManagerVehicle(); }
...
bool GeoFenceController::showPlanFromManagerVehicle(void) {
    if (_masterController->offline()) return true;
    _itemsRequested = true;
    if (!_managerVehicle->initialPlanRequestComplete() || syncInProgress()) return true;
...
void GeoFenceController::addInclusionPolygon(QGeoCoordinate topLeft, QGeoCoordinate bottomRight) {
    ...
    QGCFencePolygon* polygon = new QGCFencePolygon(true, this);
...
void GeoFenceController::addInclusionCircle(QGeoCoordinate topLeft, QGeoCoordinate bottomRight) {
    ...
    QGeoCoordinate centerTopEdge = topLeft.atDistanceAndAzimuth(halfWidthMeters, 90);
    QGeoCoordinate center(centerLeftEdge.latitude(), centerTopEdge.longitude());
    QGCFenceCircle* circle = new QGCFenceCircle(center, radius, true, this);
...
void GeoFenceController::deletePolygon(int index) {
    ...
    auto* polygon = qobject_cast<QGCFencePolygon*>(_polygons.removeAt(index));
    polygon->deleteLater();
}
void GeoFenceController::deleteCircle(int index) {
    if (index < 0 || index >= _circles.count()) return;
    auto* circle = qobject_cast<QGCFenceCircle*>(_circles.removeAt(index));
    circle->deleteLater();
}
void GeoFenceController::clearAllInteractive(void) {
    for (int i=0; i<_polygons.count(); i++) _polygons.value<QGCFencePolygon*>(i)->setInteractive(false);
    for (int i=0; i<_circles.count(); i++) _circles.value<QGCFenceCircle*>(i)->setInteractive(false);
}
...
double GeoFenceController::paramCircularFence(void) {
    if(_managerVehicle->isOfflineEditingVehicle()) return 0;
    if(_managerVehicle->px4Firmware()){
        if(!_managerVehicle->parameterManager()->parameterExists(ParameterManager::defaultComponentId, _px4ParamCircularFence)) return 0;
    ...
    if(_managerVehicle->apmFirmware()) {
        if (!_managerVehicle->parameterManager()->parameterExists(ParameterManager::defaultComponentId, _apmParamCircularFenceRadius) ||
            ...
            !_managerVehicle->parameterManager()->parameterExists(ParameterManager::defaultComponentId, _apmParamCircularFenceType)) return 0;
        ...
        if(!apm_fence_enabled || !apm_fence_type_circle) return 0;
...
void GeoFenceController::_parametersReady(void) {
    if (_px4ParamCircularFenceFact) { _px4ParamCircularFenceFact->disconnect(this); _px4ParamCircularFenceFact = nullptr; }
    if (_apmParamCircularFenceRadiusFact) { _apmParamCircularFenceRadiusFact->disconnect(this); _apmParamCircularFenceRadiusFact = nullptr; }
    if (_apmParamCircularFenceEnabledFact) { _apmParamCircularFenceEnabledFact->disconnect(this); _apmParamCircularFenceEnabledFact = nullptr; }
    if (_apmParamCircularFenceTypeFact) { _apmParamCircularFenceTypeFact->disconnect(this); _apmParamCircularFenceTypeFact = nullptr; }
    ParameterManager* _paramManager = _managerVehicle->parameterManager();
    if(_managerVehicle->isOfflineEditingVehicle()){ emit paramCircularFenceChanged(); return; }
    if(_managerVehicle->px4Firmware()){
        if(!_paramManager->parameterExists(ParameterManager::defaultComponentId, _px4ParamCircularFence)){ emit paramCircularFenceChanged(); return; }
        ...
        if (!_paramManager->parameterExists(ParameterManager::defaultComponentId, _apmParamCircularFenceRadius) ||
            ...
            !_paramManager->parameterExists(ParameterManager::defaultComponentId, _apmParamCircularFenceType)){ emit paramCircularFenceChanged(); return; }
